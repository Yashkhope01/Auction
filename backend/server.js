...
const SECRET_KEY = process.env.SECRET_KEY || 'a_default_secret'; 
...
const auctionItemSchema = new mongoose.Schema({
  ...
  currentBid: { type: Number, required: true, min: 0 }, // Ensure bid is positive
...
app.post('/auction', authenticate, async (req, res) => { // Require authentication
  try {
    ...
    const newItem = new AuctionItem({
      ...
      highestBidder: req.user.username, // Set to logged-in user
      closingTime,
    });
...
app.post('/bid/:id', authenticate, async (req, res) => { // Added authentication
  try {
    const { id } = req.params;
    const { bid } = req.body;
    ...
    // Check if the auction has ended *before* accepting the bid
    ...
    if (bid > item.currentBid) {
      item.currentBid = bid;
      item.highestBidder = req.user.username; // Set to the logged-in user's username
      await item.save();
